...
describe('Scrap informations', () => {
    it('Login & identity picture', () => {
        cy.login('{{usernameEtudes}}', '{{passwordEtudes}}')
        ...
        if (cy.get("div.errorBox_panier").should('be.empty')) {
            cy.get("#tableauDemandes")
                ...
                .each((elm) => {
                    ...
                    scrapingDto.panierFormation.push(listeFormations)
                })
        } else {
    ...
    it('Send API Request', () => {
        ...
        cy.request('POST', 'http://127.0.0.1:8000/api/scraping?usernameEtudes=Fifassigankoue@gmail.com', scrapingDto)
    })
})
